...
describe("usersController.signUp", () => {
  ...
  it("should throw an error if user already exists", async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        error: expect.stringContaining("Account for email already exists"),
      })
    );
  });
  ...
  it("should create a new user successfully", async () => {
    ...
    expect(res.locals.success).toBe(
      "Account successfully created for newuser@example.com"
    );
...
describe("usersController.verifyUser", () => {
  ...
  it("should return error if user is not found", async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ err: "User not found" })
    );
  });
  ...
  it("should return error if password does not match", async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ err: "Password does not match" })
    );
  });
...
describe("usersController.getUsers", () => {
  ...
  it("should return user data if user exists", async () => {
    ...
    db.query.mockResolvedValueOnce({
      rows: [{ firstname: "John", email: "test@example.com" }],
    });

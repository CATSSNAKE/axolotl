...
usersController.signUp = async (req, res, next) => {
  ...
  if (!email || !password)
    return next({ error: "Email and Password required!" });
  ...
  try {
    ...
    if (activity && Object.keys(activity).length > 0) {
      const activityQueries = Object.entries(activity).map(
        ([activityName, skillLevel]) =>
          db.query(
...
usersController.getUsers = (req, res, next) => {
  db.query("SELECT * FROM users")
    ...
    .catch((err) =>
      next({ error: `${err.message} in usersController.getUsers` })
    );
};
...
usersController.getFilteredUsers = (req, res, next) => {
  ...
  db.query(
    ...
    .catch((err) =>
      next({ error: `${err.message} in usersController.getFilteredUsers` })
    );
};
...
usersController.deleteUser = (req, res, next) => {
  ...
  db.query("DELETE FROM users WHERE email = $1 RETURNING *", [email])
    ...
    .catch((err) =>
      next({ error: `${err.message} in usersController.deleteUser` })
    );
};
